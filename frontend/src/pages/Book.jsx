...
const Book = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/v1/auth/book', {
                method: 'POST',
                headers: {
                ...
                <div className="booking-form">
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        <div>
                            ...
                            <input type="text" id="name" name="name" value={bookingDetails.name} onChange={(e) => setBookingDetails({ ...bookingDetails, name: e.target.value })} placeholder="Your booking name..." className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required />
                        </div>

                        <div>
                            <label htmlFor="date" className="block text-sm font-medium text-gray-700">Travel Date:</label>
                            <input type="date" id="date" name="date" value={bookingDetails.date} onChange={(e) => setBookingDetails({ ...bookingDetails, date: e.target.value })} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required />
                        </div>
